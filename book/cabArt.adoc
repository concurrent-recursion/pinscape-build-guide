...
In the 1990s, the manufacturers started using a multi-color silk-screening process, which allowed for higher-resolution graphics with more detailed designs.
...
(Silk-screening's palette is limited by the number of color layers used, and has to use half-tone patterns for in-between colors.) The switch to decals opened up even more options for art designers, including full photo-realism.
...
I've had less than perfect results that way myself, and all of the kitchen cabinet pros seem to agree that grain fillers are a must if you want a piano finish, but maybe it's a matter of finding the right paint.
...
Only do this with with oil-based paints.
...
For example, if you like space themes, check the NASA, JPL, and Hubble Space Telescope Web sites for some very pretty, high-resolution astronomy images that are free to use.
