...
(To be really technical, there's also a limit on the combined power across groups of ports, but we can mostly ignore that in a pin cab; that only applies if all of the ports are switched on for long periods at the same time, which essentially never happens in a pin cab.)
...
What should you do if a device exceeds the 500mA limit? Well, for starters, *don't* connect it directly to the LedWiz.
...
* All of the "-" or "Ground" connections from all of your power supplies should be connected together (see xref:powerSupplies.adoc#powerSupplies[Power Supplies for Feedback] )
...
All of the other common devices have much lower power needs.)
