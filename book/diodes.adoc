...
It's important to install diodes on these devices, because all of them have magnetic coils that generate voltage spikes as part of their normal operation.
...
Why? Think about what would happen if it were installed the other way: when the power goes on, the diode would happily conduct all of the current straight through, bypassing the load.
...
All of the power goes through the load (the feedback device) just like we want it to, and nothing gets fried.
