...
But it's also okay to use a single power supply for now, since we can do all of the testing with low-power devices.
...
If you buy a set of these on eBay, they'll often come as a sort of "ribbon cable", with all of the wires joined together in a flat bundle.
...
The first thing to check is that all of the buttons are showing their Pin Status as *OFF* .
...
All of the buttons have to go through that, so it's the prime suspect when the whole thing isn't working.
...
All of the Small LED outputs are controlled by TLC5940 #2, so find the port listed in the Pin column as *TLC5940 #2 OUT0* .
...
The Strobe output is wired to the same chip that handles all of the RGB Flasher outputs, so you can test it almost exactly like the flasher outputs.
...
If you've been following our procedures so far, you've now tested all of the major systems on the main board at this point, so you can move on to the power board.
...
You might wonder: why are we using +5V flasher pin from the _main_ board to test the _power_ board outputs? There's nothing magic going on here.
