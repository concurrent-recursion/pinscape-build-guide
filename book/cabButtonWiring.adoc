...
These work exactly like keyboard encoders in terms of all of the physical setup and wiring, so everything in this chapter applies equally well to joystick encoders.
...
All of the key encoders (commercial and DIY) use the same basic wiring plan, so we'll show you how this works in general.
...
What about buttons with more than two terminals? No problem: you still only connect two wires.
...
All of the commons are connected together electrically, so it doesn't matter whether they connect back to the encoder with a separate wire or if they connect to each other in a daisy chain like this.
...
I'd use one color for all of the Common/Ground wires - black, if you want to follow the normal conventions - and a mix of other colors (never the same as the Common/Ground color) for the unique button port wires.

See xref:wire.adoc#wire[Wire] for more on wire selection.

== Software setup

All of the key encoders emulate either a keyboard or a joystick, so there's no need for Windows device drivers.
...
Fortunately, almost all of the PC pinball simulators have used the same core key assignments for decades, so you won't have fight with the software too much if you stick to the standard keys.
...
What about the lamp terminals? That's a whole separate subject, because even though the lamp is part of the button assembly, it's controlled by a whole separate electrical system on your virtual cab.
...
You can connect all of these switches to your key encoder in the same way as other buttons.
Visual Pinball has keyboard equivalents for all of these functions, so connecting these switches lets you access each function the same way you'd do it on a real machine.
...
All of the "1" terminals are part of the first switch, and the "2" terminals are part of the second.
