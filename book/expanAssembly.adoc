...
The boards can look pretty daunting to build, because there are admittedly a lot of components to install, but all of the components are individually pretty easy to handle.
All of them are "through-hole" parts, with wire leads that install through holes in the board.
...
All of the markings on the circuit board are there to let you see at a glance what goes where.
...
See how the word STROBE is written next to it, with an arrow? A lot of people understandably take this to mean "Hey, guys, the STROBE is over HERE!" But it doesn't mean that at all.
...
All of the Small LED ports have the same current limit, but you can adjust what that common limit is, because it's determined by the value of resistor *R5* .
...
Why might you want to use two emitters? To control two TVs! If you use two emitters, you can place one near each TV's remote control receiver, to ensure that both TVs get strong signals.
...
To remove it, simply omit all of the following parts:
...
The list above includes all of the parts that make up both the knocker's time-limiter circuit and the output control circuit, so if you omit all of those parts, you can skip the section below about bypassing the knocker output timer.
...
This means we're going to basically *omit* all of the normal parts.
