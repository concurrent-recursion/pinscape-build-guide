...
*Important:* Pay special attention to the "linear taper" part.
...
The fifth file, *Combined Parts.stl* , contains all of the other parts joined together into a single contiguous 3D object, using the "model airplane" style where the piece are connected by little plastic struts.
The combined parts file lets you manufacturer all of the parts in a single printer run.
...
This bracket attaches between the pot and the side wall of the cabinet.
