...
This is probably my favorite plunger sensor out of all of the current options. 
...
The plans presented below include parts lists, 3D-printable designs for the mounting bracket, and a custom circuit board layout with all of the interface electronics, so you won't have to improvise any of the elements yourself. 
...
You just heat up the whole board to solder-melting temperatures, and all of the parts are simultaneously soldered to their proper places. 
It's almost magic. 
...
This is a plastic or metal sheet with holes etched to match all of the SMD pin pads on the board. 
...
All of the normal procedures needed for any static-sensitive parts apply (see xref:staticSafety.adoc#staticSafety[Static Electricity Precautions] ), but I'd be extra vigilant with this part because of its high sensitivity.
...
Please observe all of the precautions diligently - the TCD1103 can be easily destroyed by improper handling.
...
Tape it down to the work surface (leaving all of the holes uncovered, of course).
...
Overexposing a CCD can cause a cascade effect that maxes out all of the pixels and gets them stuck like that until you reset power. 
...
If the sensor doesn't respond as expected, the first troubleshooting step is to confirm that all of the GPIO pins for the sensor are assigned correctly in the Settings page. 
...
Overexposing the sensor beyond a certain point can cause a cascade effect that blinds all of the pixels, which can last until you power cycle the chip. 
...
Check all of the solder joints carefully (I'd use a magnifying glass). 
...
How precise do we have to be? The sensor's field of view side-to-side is only one pixel wide, which translates to a fraction of a millimeter along the plunger. 
...
The plunger shaft usually has a little bit of play that lets it move around by a couple of millimeters in normal action, plus you have to expect all of the parts to move around a little bit over time.
...
* What if you simply can't get both ends of the range in view, because the field of view isn't wide enough for the travel range? This means that the sensor is too close to the plunger. 
...
* What if the bright region starts disappearing halfway along the travel range? This probably means that the sensor's pixel window isn't close enough to parallel to the plunger, so that the plunger moves "diagonally", relative to the sensor, as you pull it back. 
