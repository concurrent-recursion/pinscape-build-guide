...
How do you fix a table that ignores the joystick input? Unfortunately, it's not easy - but it can be done. 
Basically, you have to rip out all of the scripted plunger code in the table, and substitute the "real" built-in plunger object instead. 
...
The second nice thing is that you don't really have to write any new code - mostly it's just a matter of deleting all of the custom plunger code so that the built-in plunger can take over. 
...
Remember how I said that the built-in plunger in older versions of VP had bad physics? The situation was so bad that the bad physics actually interfered with ordinary desktop use, even if you didn't have a joystick attached. 
...
There's also usually a bunch of other stuff overlapping in the same area, so even if the wedge is there, it might be hard to visually separate from all of the other objects. 
...
The custom scripted graphics in many older tables look okay, and some look great, but almost all of them have really choppy animation with just a few "stops" the plunger can appear at.
...
A lot of tables with plunger scripts are actually pretty helpful about grouping all of the plunger scripts together in one place. 
...
Specifically, you can rely on the fact that all of the plunger scripts will probably make reference to the playfield objects we just deleted. 
