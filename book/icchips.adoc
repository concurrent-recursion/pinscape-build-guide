...
How do you pick out the chip name from the other stuff? You pretty much have to figure it from context, by looking for a chip name you recognize.
...
When you look at the Pinscape boards, you'll see little outlines for all of the chips printed on the top of the board.
...
It only takes a tiny bit of solder on each pin to make it stick, and it's next to impossible to get all of the solder off all of the pins on a large chip.
...
If you're using a socket, follow the procedure we're about to describe for soldering the chip, and just substitute the socket for the chip in all of the steps.
...
Once you're satisfied that all of the pins made it into their respective holes in the board, hold the pin in place from the top and flip the board over.
Verify that all of the pins really made it through the openings, as seen from the bottom of the board.
...
Those pins are all of the PWM outputs.
...
(All of the light transmission happens inside the chip, so you won't see light coming out of it, and you don't have to worry about interference from ambient light.
...
In my own schematics, I also make a point of grouping the sub-units together on the page, so that you don't have to go hunting around to find all of them, but you might encounter cases in other people's schematics where the sub-units aren't grouped.
