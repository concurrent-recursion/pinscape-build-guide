...
However, the measurements assume that all of the pieces extend all the way to the corners of the finished cabinet, which is only true when you're using one of the mitered joins (such as a simple 45° miter, a locking miter, or a mitered rabbet).
...
At this point, I like to mark all of the panels with the locations of the routed grooves, cutouts, and drills - essentially everything below.
...
Mark the locations of the three bolt holes.
Remove the receiver and measure the distance from the top edge of the panel to the center of the marked bolt hole locations.
...
You should also adjust all of the following up or down by the same amount to match, since all of these pieces fit together when installed:
...
The flipper button goes at the same location whether or not you're including a MagnaSave button.

For System 11 rails or other wide rails that cover the flipper buttons, don't use these coordinates.
Instead, drill at the same location as the pre-cut button hole in the rail, using the rail itself as a template.
...
See the *Alternative Procedure* under xref:html#postAssemblyBackboxHinges[Post assembly: drill the backbox hinge bolts] below.
...
link:https://pinside.com/pinball/forum/topic/bally-wms-cabinet-designs-help-needed.html[Swinks's plans on Pinside] make the same assumption, although they differ from mine on the exact locations of the drills by 1/8" to 1/4".
...
See xref:html#postAssemblyBackboxHinges[Post assembly: drill the backbox hinge bolts] below.
...
These drills are for #10 carriage bolts, which fasten the insert hinge brackets, parts A-12497 (upper) and A-12498 (lower), to the inside of the left wall of the backbox.
...
Rotate the hinges up so that they sit flat against the bottom of the backbox, and mark the locations of the three bolt holes.
...
*Alternative procedure:* Drill the six bolt holes in the backbox floor first, but *do not* drill the pivot bolt holes in the cabinet sides yet.
