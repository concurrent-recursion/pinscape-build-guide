...
Which speed class is best for a pin cab? It depends on the type of circuit you're protecting.
...
Here are example holders for the most common sizes (these cover all of the fuses linked in this chapter):
...
That lets you do all of the initial wiring on your workbench rather than in the confined spaces inside your cab.
...
You'll want to use some kind of mating plug-and-socket connector to connect all of the wires coming out of the fuses to the devices inside the cabinet.
...
That's a connector from the link:https://www.mouser.com/search/ProductDetail.aspx?R=538-03-06-1122.html[Molex .062"] series, which I found useful for many of the interconnects in my cabinet.
...
The wiring for a fuse in an output circuit is the same for all of the controllers, so we'll just show the LedWiz as an example.
...
Why are we even worried about this? We only use these coils to simulate bumpers and other things that fire momentarily, so why would the software ever leave them on for long periods? Normally, it wouldn't.
...
All of the outputs on the Pinscape chime board also have these timers.
...
But how long is too long? Unfortunately, I don't have any hard data on that.
...
Here's the part number and a Mouser link: link:https://www.mouser.com/search/ProductDetail.aspx?R=530-3JS1.5-RTR.html[Bel Fuse 3JS 1.5-R TR] .
...
How do they accomplish this, when other coils can't? Their trick is a clever two-coil design.
