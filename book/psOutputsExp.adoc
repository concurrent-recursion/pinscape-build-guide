...
* As described in xref:powerSupplies.adoc#powerSupplies[Power Supplies for Feedback] , you should already have connected the power supply's negative terminal to the common ground interconnect for all of the feedback power supplies
...
*Main board JP9 (Knocker/Strobe)* has one pin dedicated to the replay knocker, and one pin dedicated to a "strobe" output. These assignments are printed on the board next to the pin header to help you identify which is which. See xref:contactors.adoc#contactors[Flippers, Bumpers, and Slingshots] and xref:flashers.adoc#flashersAndStrobes[Flashers and Strobes] for notes on these device types.
...
*Main board JP8 (Small LED/Opto)* has 16 pins designed for low-power LEDs. These are intended primarily for controlling LEDs in the flipper and MagnaSave buttons; see xref:buttonLamps.adoc#buttonLamps[Button Lamps] for how to set those up.
...
All 8 of the Chime Board outputs are high-power outputs with time-limiter circuits of the same type used in the replay knocker output on the main board. As with the replay knocker, these are designed to protect pinball coils from software faults on the PC that could leave an output stuck on. They're nominally for "chime units" (see xref:chimes.adoc#chimes[Chimes and Bells] ), but they're really appropriate for any sort of standard pinball coil, such as bumpers and slingshots. You wouldn't want to use them with flippers, however, as you certainly do want to allow flippers to be held on for long periods.
...
To set up your Pinscape unit with DOF, you of course have to install DOF on your PC first. See xref:DOF.adoc#dofSetup[DOF Setup] for instructions.
