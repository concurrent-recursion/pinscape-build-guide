...
Before connecting of disconnecting any of the expansion board cabling, you should have the power off to the PC and all of the power supplies.
...
All of the expansion boards have *two* power input: one labeled *PC PSU* and one labeled *2ND PSU* .
...
All of the buttons connect to the Common pin (thus the name!), but each button gets its own individual numbered input pin.
...
One nice thing about these crimp pin housings is that you can insert pins one at a time, so there's no need to install all of the pins at once.
...
The one pin you probably will want to install up front is the Common wire, since all of the buttons will need to connect to that.
