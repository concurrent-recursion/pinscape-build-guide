...
The connectors on the boards use special cables that you can build yourself. See xref:expanPower.adoc#expanPower[Expansion Board Power Cables] for full instructions.
...
The easiest and best way to connect to this header is using a ribbon cable. There's a separate chapter for each type of plunger sensor, and each contains instructions for building the ribbon cable connector for that sensor. Also see xref:ribbonCables.adoc#ribbonCables[Ribbon Cables] for general instructions on building ribbon cables.
