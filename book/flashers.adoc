...
What's magical about the numbers 3 and 5? History, mostly.
...
All of the parts I've seen for these on eBay are the same basic device, so there's not too much risk of finding a wrong part that looks similar, but here are the approximate electrical specs to look for just to be sure you have the right thing:
...
Does this mean that you're supposed to find a 2.2V power supply? Fortunately not! The "forward voltage" for an LED *isn't* the supply voltage required.
...
What does this mean for the supply voltage, then? It's really simple: the supply voltage can be _any voltage higher than_ the forward voltage.
...
Using a 12V supply just burns up all of the extra voltage in resistor heating, which forces you to use physically larger resistors that can tolerate all of the extra waste heat.
...
* I soldered four wires to each of the five star LEDs: one wire to each of Red(-), Blue(-), and Green(-), and a fourth wire that I connected to all of the (\+) terminals.
