...
Getting B2S working takes a few additional steps beyond just installing the software. We cover the details in xref:b2s.adoc#b2s[Backglass Software Setup] .
...
This is my own project, brand new in late 2018. I tried to make it easy and quick to set up so that you can try it out without a lot of hassle. It's designed specifically for pin cabs, and has built-in integration with most of the pin cab ecosystem, including xref:DOF.adoc#dofSetup[DOF] , real DMD devices, joysticks (for button input), and multiple monitors. It's also highly customizable via a built-in Javascript scripting engine.
