...
So how exactly do we insert an output controller into this picture? The exact details can vary a little bit depending on the type of controller you're using - and we'll show you those exact details for each type of controller in the chapters ahead - but the principle is always the same.
...
But what about devices that run on different power supplies that use other voltages? If you already read through xref:powerSupplies.adoc#powerSupplies[Power Supplies for Feedback] , you know that you might need as many as five or six different voltages if you're using a diverse mix of devices:
...
So how do you hook up all these different devices, when each one has to be connected to its own power supply? What happens to that common wire back to the (-) on the power supply when we have five power supplies?
