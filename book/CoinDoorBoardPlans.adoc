...
That plug is wired to all of the switches and lamps in the coin door: the coin chute switches that detect when you've inserted a quarter, the slam tilt switch, the service control panel buttons, and the coin slot lamps.
...
You can simply omit all of the parts for it (leaving their slots on the circuit board empty).
...
The old design is still available, of course; see xref:html#CoinDoorBoardVersion1[Version 1] below.
...
*Screw terminals or pin headers:* The parts list calls for Phoenix Contact screw terminals for JP1 and JP2, which are the places where you connect the wiring that goes out to your key encoder and power supply.
...
If you already have a separate 6.3V supply, you can omit all of these and instead supply the board with 6.3V from the external supply.
...
*To build it _without_ the 6.3V regulator:* Don't install any of the voltage regulator parts (C1, C11, Q1, or the 12V input pin header).
