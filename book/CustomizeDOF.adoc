...
The best way I've found to reach them is to post the request to the following forum on link:https://vpuniverse.com/forums/forums.html[VPUniverse] : Digital Pinball Cabinets > Direct Output Framework > Direct Output Support.
...
* In the right column of each row, simply delete all of the text in the edit box, so that the box is complete empty.
This removes all of the events that would cause the flipper solenoids to fire while you're running the front-end program, which will have the effect is disabling the solenoid effects entirely.
...
The table script (written in Visual Basic, assuming we're talking about VP) contains all of the game's logic, including all of the scoring rules and sound effects.
...
If you can't find this stuff in the Visual Basic scripts, where do you look for it, then? Well, we _could_ look at the Visual Basic equivalent in these games - namely the ROM scripts themselves.
...
The manual generally has lists of all of the switches, lamps, and solenoids, as well as instructions on how to use the game's operator menu to adjust game settings and run diagnostics.
...
These were used in almost all of the games in the 1980s and 1990s, so you'll see this first column all the time.
...
VPinMAME handles these switches by assigning them to a numeric range above all of the matrix switches, starting at 111.
...
But how do we get the solenoids to fire? We're going to use the game's own built-in test menu.
...
The diagnostics usually include a solenoid test mode, which lets you cycle through all of the solenoids in the game and fire each one.
...
Write down each mapping and repeat until you've visited all of the solenoids.
...
You'll know you're in test mode when you start seeing all of the numeric displays cycle through a sequence like this: 0000000, 1111111, 2222222, 3333333...
