...
*  *Power button:*  _Essential._ Pushbutton, bottom of cab, front right side. +On the real machines, the power is controlled with a toggle switch wired to the main power supply line, located in a little recess on the bottom of the cab at the front right corner. In a virtual cab, we usually replace this with a "soft" power button in the same location, wired to the PC motherboard's power control header. See "Floor" in xref:cabBody.adoc#softPowerButton[Cabinet Body] for details on installing a pushbutton in the proper spot, and xref:powerSwitching.adoc#powerSwitching[Power Switching] for more on setting up the PC soft power controls.
