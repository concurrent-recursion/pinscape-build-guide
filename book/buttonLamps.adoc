...
Some pin cab builders simply wire all of the button lamps so that they're always on.
...
All of the common buttons are specifically supported in the software, so all you have to do is set up your DOF configuration so that DOF knows which output controller port is connected to each button lamp (the Start button lamp, Launch Ball button lamp, etc).
...
so does that mean that the input encoder also controls the button lights? No.
...
(What does it actually mean for DOF to support RGB vs monochrome for a given output? In terms of hardware, full-color lighting is accomplished with a three-component LED: an LED with individually controlled elements for red, green, and blue light.
...
(LEDs _are_ polarized as a rule, but nearly all of the #555 replacement bulbs are specially designed to work with either polarity, since they're designed to replace incandescents in existing machines.)
...
On my own cab (and on all of the "real" pinball machines I've seen), they're simply attached directly to the side wall of the cabinet.
...
As of June 2021, I don't think Oak Micros is selling any of their products any more, but you might check the original vpforums thread for any updates:
...
I used these for my own build and would be happy to recommend them, but unfortunately they don't seem to be available any more.
