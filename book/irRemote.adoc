...
* Connect GND to one of the ground pins on the KL25Z. See xref:kl25zPinOut.adoc#kl25zPinOut[KL25Z Pin Out] .
...
* Connect *3V3_KL25Z* to one of the 3.3V pins on the KL25Z. See xref:kl25zPinOut.adoc#kl25zPinOut[KL25Z Pin Out] .
* Connect GND to one of the ground pins on the KL25Z. See xref:kl25zPinOut.adoc#kl25zPinOut[KL25Z Pin Out] .

[#irPinSelection]
=== Selecting GPIO pins for the power sensing circuit

The IR transmitter can use any PWM-capable GPIO pin on the KL25Z. The default for the expansion boards is PTC9, but you can use any pin with PWM capability. For a list of PWM pins, see xref:kl25zPinOut.adoc#kl25zPinOut[KL25Z Pin Out] . The Config Tool will also show you the available pins if you go to the IR section on the Settings page and click on the "IR LED (transmitter) pin" box:

image::images/IR-tx-config.png[""]

The IR receiver can use any interrupt-capable GPIO pin. The default for the expansion boards is PTA13, but you can use any pin in the "PTAxx" or "PTDxx" groups. See xref:kl25zPinOut.adoc#kl25zPinOut[KL25Z Pin Out] for all interrupt-capable pins, or click on the "IR receiver input pin" box in the IR section on the Settings page in the Config tool:
