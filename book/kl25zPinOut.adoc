...
All of the channels on a given module share the same period (or, equivalently, frequency).
...
Remember, the frequency of a PWM port is set at the *unit* level, so all of the ports associated with TPM Unit 0 will operate at the same frequency, all of the ports assigned with Unit 1 will operate a second common frequency, and all of the ports on Unit 2 operate on a third common frequency.
