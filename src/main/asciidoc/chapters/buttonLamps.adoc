...
If you're not sure what all of the buttons are for, see xref:cabButtons.adoc[Cabinet Buttons] for more detailed descriptions of the various button types commonly found on virtual cabs.
...
image::resources/ButtonWiring.png[""]

== Start, Exit, Extra Ball

image::resources/StartExitExtraBall.png[""]
...
image::resources/FrontPanelButton.png[""]
...
image::resources/MicroswitchVsButtonPlusLamp.png[""]
...
image::resources/SwitchWithLampDiagram.png[""]


WARNING: The "Common" and "NO" (Normally Open) might not be positioned exactly as shown above for your switch. Double-check the markings on the switch to be sure before connecting anything. See xref:cabButtonWiring.adoc[Button Wiring] for tips on reading switch markings.
...
image::resources/ButtonLampWiring.png[""]
...
See xref:cabButtons.adoc[Cabinet Buttons] for more detailed descriptions of the button types.
...
* Connect the other terminal of each lamp to an available port on your output controller (following the standard wiring plan in xref:feedbackWiring.adoc[Feedback Device Wiring] )

image::resources/ButtonLampWiringWithConverter.png[""]

== Launch Ball button

image::resources/PlungerAndLaunchButton.png[""]
Several real machines from the 1990s used big round "Launch Ball" buttons in place of plungers. Many pin cab builders use the same buttons, either instead of plungers or in addition to plungers. See xref:plunger.adoc[Plunger] for more on choosing whether to include one or the other or both, and where to place the Launch button if you're including one.
...
* ZB Launch Ball: ignore this one. It's not for button lamps, but for something else entirely (see xref:zblaunch.adoc[ZB Launch Ball] ).
...
image::resources/LightedFlipperButtons.png[""]
...
link:http://mjrnet.org/pinscape/downloads/Flipper_Button_LED_board.zip.html[mjrnet.org/pinscape/downloads/Flipper_Button_LED_board.zip]
...
image::resources/LightmiteBoards.png[""]
...
image::resources/LightmiteInstall.png[""]
...
image::resources/LightmiteFinal.png[""]
...
The pin headers and crimp pin housings are the same types used in several places in the expansion boards; see xref:partslist.adoc[Electronic Parts List] and xref:crimpPins.adoc[Crimp Pins] .
...
If you're using a real coin door, you can find more information about how the coin chute lights are wired (and how to access that wiring) in xref:coinDoor.adoc[Coin Door] .
