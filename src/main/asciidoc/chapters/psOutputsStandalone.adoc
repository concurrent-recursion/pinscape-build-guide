...
WARNING: A pin can only be assigned to one function at a time. The dialog will allow you to select pins that are already assigned to other functions, but if you do, the pin will show a little error icon (image:resources/pinwarn.png[""]) next to the pin. You *must* resolve any such warnings before the device will work properly.
...
image::resources/kl25zBoosterGeneric.png[""]
...
Note that the diagram above also leaves out other details specific to the type of output device. For an LED, you'd need a current-limiting resistor somewhere in the wiring; see xref:ledResistors.adoc[LED Resistors] . For a coil, solenoid, motor, etc., you'd need a diode to protect against the magnetic field effects; see xref:diodes.adoc[Coil Diodes] . See the descriptions of the individual types of feedback devices for more details on each one's special needs.
...
image::resources/kl25zBoosterNPN.png[""]
...
image::resources/kl25zBoosterMOSFET.png[""]
...
If you're connecting a device with any sort of coil (including solenoids, relays, contactors, and motors), be sure to use a diode with the device, as explained in xref:diodes.adoc[Coil Diodes] .
...
image::resources/LedStripAmpH.jpg[""]
...
xref:[The online]  link:https://configtool.vpuniverse.com/[DOF Config Tool] has a confusing bit of terminology in the device setup section on the "My Account" page. In the list of devices, you'll find separate entries for "Number of Pinscape devices" and "Number of KL25Z devices".
