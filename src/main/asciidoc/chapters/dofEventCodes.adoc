...
image::resources/dof-config-tool.png[""]
...
If you're happy with the default DOF effects provided by the Config Tool's database, and you don't want to make any customizations of your own, then you don't have to know anything about how DOF works apart from the basics of installing it on your system, which we covered separately in xref:DOF.adoc[DOF Setup] . But if you do want to customize anything, you'll have to learn what the cryptic DOF codes mean. DOF's documentation has some of this information, but it's incomplete and (in my opinion) poorly organized and almost as cryptic as the codes themselves. So this section is my attempt at a more approachable and more thorough explanation of what the codes mean and how to use them.

The actual procedure for customizing tables in the Config Tool is covered separately, in xref:CustomizeDOF.adoc[Customizing a table's DOF effects] . The present section covers just the syntax that you enter in the Config Tool to make those customizations.
...
image::resources/dof-config-tool-2.png[""]
...
|L _number_ 
|L7
|Lamp. The toy is activated when the given lamp on the table is on. Lamps refer to the individually controlled lights on the playfield or backbox, such as an "Extra Ball When Lit" light or a bumper lamp. The lamp numbers are defined by the particular table, according to the game's original ROM programming. See xref:CustomizeDOF.adoc[Customizing a table's DOF effects] for tips on figuring out which lamp is which for a given table.
