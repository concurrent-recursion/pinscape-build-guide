...
* Select your sensor type from the drop list image:resources/plunger-sensor-select.png[""]
...
image::resources/plunger-setup.png[""]

This will bring up the plunger sensor viewer.

image::resources/plunger-viewer.png[""]
...
image::resources/plunger-reverse-orientation.png[""]
...
If you installed a dedicated calibration button, you can also calibrate using the button instead of going through the Config Tool UI. There's no good reason to do that, as the Config Tool has a friendlier UI that shows more information about what's going on, but the button will work, too, if you prefer to use that for some reason. The procedure is described in xref:calbtn.adoc[Pinscape Plunger Calibration Button] .
...
image::resources/jitter-filter-setting.png[""]
...
image::resources/jitter-window.png[""]
...
image::resources/Pinscape-joystick-axis-setting.png[""]
