...
image::resources/10BumperPlacement.png[""]
...
image::resources/8BumperPlacement.png[""]
...
*Power supply:* The Siemens contactors require 24VDC power. That's not available from a computer ATX power supply, so you'll need a separate power dedicated 24V supply. See xref:powerSupplies.adoc[Power Supplies for Feedback] for advice.
...
image::resources/SiemensContactorWiring.png[""]
...
The diode is required to protect the output controller and computer electronics from noise from the magnetic coil inside the contactor. See xref:diodes.adoc[Coil Diodes] . The stripe on the diode must connect to the positive (+) power side.
...
image::resources/StarterRelayWiring.png[""]
...
*Wiring:* Each coil will have two terminals for the power connections. Connect these following the generic output device wiring plan (see xref:feedbackWiring.adoc[Feedback Device Wiring] ).
...
image::resources/RealPinballCoilWiring.png[""]

If a diode _isn't_ already installed, you must add one. See xref:diodes.adoc[Coil Diodes] . When a diode isn't present, the coil itself will be unpolarized, so the order of the wires doesn't matter. Just be sure that the diode you add is installed with the correct polarity.
...
image::resources/OpenFrameSolenoid.png[""]
...
image::resources/OpenFrameSolenoidWiring.png[""]
...
* Click the flipper icon (image:resources/FlipperLogicOff.png[""]
...
image::resources/flipper-button-opto-to-solenoid.png[""]
...
The diode is needed to protect the rest of the circuit from the voltage surge effect from the contactor or solenoid. A diode like this is needed for every device with a coil or motor. See xref:diodes.adoc[Coil Diodes] for more details.
