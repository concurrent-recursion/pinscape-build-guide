...
I personally wouldn't go out of my way to include a slam tilt switch on a virtual cabinet, but if you install a real coin door, it'll probably have one built-in. Visual Pinball and PinMAME have support for this switch as an input, so you can attach it if you wish. See xref:coinDoor.adoc[Coin Door] for more on that.

== Virtual nudging, part 1: the sensor

image::resources/kl25zSmall.png[""]
...
The commercial plunger kits come ready to use out of the box. For the Pinscape controller, you'll have to install the Pinscape firmware, described in xref:kl25zSoftwareSetup.adoc[KL25Z Software Setup] .
...
image::resources/kl25zOrientation.png[""]
...
image::resources/HistoryEraserButton.png[""]
...
image::resources/vpAccelSettings.png[""]
...
The best option for a tilt bob is to buy a real one from a pinball parts supplier (see xref:resources.adoc[Resources] ). They cost about $10 to $20. You could also fashion one yourself, but the real ones are cheap enough that it's probably not worth the added effort.
...
Button wiring is described in more detail in xref:buttons.adoc[Pinscape Button Inputs] .
...
** Download link:http://mjrnet.org/pinscape/downloads/NudgePlugIn.vbs.html[NudgePlugIn.vbs] and place it in the *Tables* folder (make sure the filename is *NudgePlugIn.vbs* )
