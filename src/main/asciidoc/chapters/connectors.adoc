...
image::resources/shroudedHeader.png[""]
...
Those are crimp-pin housings, so see xref:crimpPins.adoc[Crimp Pins] for instructions on how to assemble them.

Also see xref:powerSupplies.adoc[Power Supplies for Feedback] for more on using ATX power supplies for feedback device power.
...
image::resources/molex-062-plugged.png[""]
image::resources/molex-062-unplugged.png[""]
...
These are crimp-pin systems, so see xref:crimpPins.adoc[Crimp Pins] for instructions on assembling them.
